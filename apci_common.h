...
#define APCI_DEFAULT_DEVFILE_MODE \
	0000 // /dev/{cardname} will get these permissions on creation
...
#ifdef A_PCI_DEBUG
#define apci_debug(fmt, ...)                            \
	if (A_PCI_DEBUG > 0) {                          \
		pr_err(APCI_PREFIX fmt, ##__VA_ARGS__); \
	}
#define apci_devel(fmt, ...)                            \
	if (A_PCI_DEBUG > 1) {                          \
		pr_err(APCI_PREFIX fmt, ##__VA_ARGS__); \
	}
#define apci_info(fmt, ...)                              \
	if (A_PCI_DEBUG > 2) {                           \
		pr_info(APCI_PREFIX fmt, ##__VA_ARGS__); \
	}
#else
#define apci_debug(fmt, ...)                              \
	if (0) {                                          \
		pr_debug(APCI_PREFIX fmt, ##__VA_ARGS__); \
	}
#define apci_devel(fmt, ...)                              \
	if (0) {                                          \
		pr_debug(APCI_PREFIX fmt, ##__VA_ARGS__); \
